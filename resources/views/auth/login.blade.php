  ...
  <div class="min-h-[75vh] flex items-center justify-center">
    <div class="w-full max-w-md py-8">
      ...
      <div class="text-center">
        <p class="hero-item text-xs font-medium uppercase tracking-[0.35em] text-gray-400 mb-3" style="--d:.1s">Account</p>
        <h1 class="hero-item font-serif text-4xl sm:text-5xl font-bold text-gray-900 dark:text-white mb-3" style="--d:.2s">Welcome Back.</h1>
        <p class="hero-item text-gray-500 dark:text-gray-400 font-light mb-10" style="--d:.3s">Sign in to continue your wardrobe journey.</p>
      </div>

      <form method="POST" action="{{ route('login') }}" class="hero-item" style="--d:.4s">
        @csrf
        <div class="mb-8">
        ...
        <div class="mb-4">
          ...
          <div class="relative">
            ...
            <button type="button" onclick="togglePassword()" tabindex="-1" class="absolute right-0 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors p-2" aria-label="Show password">
        ...
        <div class="flex items-center justify-between mb-10 text-sm">
          <label class="flex items-center gap-2 text-gray-500 dark:text-gray-400 cursor-pointer select-none">
            <input type="checkbox" name="remember" class="w-4 h-4 rounded-sm border-gray-300 dark:border-gray-600 text-gray-900 focus:ring-gray-900 dark:focus:ring-white">
            Remember me
          </label>
          @if (Route::has('password.request'))
            <a href="{{ route('password.request') }}" class="text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white underline underline-offset-4 decoration-gray-300 transition-colors">Forgot password?</a>
          @endif
        </div>
        ...
        <p class="text-center text-sm text-gray-500 dark:text-gray-400 mt-10">
          ...
          <a href="{{ route('register') }}" class="text-gray-900 dark:text-white font-semibold underline underline-offset-4 decoration-gray-300 hover:decoration-gray-900 dark:hover:decoration-white transition-all">Register here</a>
        </p>
      </form>
    </div>
  </div>
