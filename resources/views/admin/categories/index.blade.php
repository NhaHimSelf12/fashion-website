            ...
            <form action="{{ isset($category) ? route('admin.categories.update', $category->id) : route('admin.categories.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    @if(isset($category) && $category->image)
                        <div class="mb-2">
                            <img src="{{ asset($category->image) }}" alt="{{ $category->name }}" class="w-24 h-24 object-cover rounded-lg shadow-sm">
                        </div>
                    @endif
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody>
                    ...
                    <tr class="border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:bg-gray-800/50">
                        ...
                        <td class="py-3 px-4">
                            @if($cat->image)
                                <img src="{{ asset($cat->image) }}" alt="{{ $cat->name }}" class="w-12 h-12 object-cover rounded-lg shadow-sm">
                            @else
